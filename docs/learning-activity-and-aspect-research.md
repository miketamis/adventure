...
## Honest mechanic inventory

The strongest gaps found in the activity sweep now have direct production paths: demonstrative-plus-noun gender retrieval, adjective/linking-article agreement, explicit “a / the / plural” form contrast before spelling, ending-focused construction, audio-to-written/meaning matching, reviewed Albanian real-word sound discrimination, linked multi-gap agreement, point-to-target before analysis, and reviewed-form odd-one-out practice across number and definiteness. Each appears in the debug matrix only through the same production family, aspect, and gate used by Train. Handwriting or script tracing remains explicitly not appropriate: Albanian uses the Latin alphabet, while ë/ç, digraphs, sound-to-spelling and typed production are the relevant demands.
