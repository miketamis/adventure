...
// Explicit tables that OVERRIDE the auto inference (irregulars, and a few
...
const flagged = []
const tables = {} // id -> rows (auto), later overridden by OVERRIDES
for (const id of nouns) {
  ...
  tables[id] = rows
  if (notes.length) flagged.push(id)
}

// merge: explicit overrides replace auto output; drop the deferred (broken-lemma) nouns
const order = ['indefNom', 'indefAcc', 'defNom', 'defAcc', 'defDat', 'plIndef', 'plDef']
const final = {}
for (const id of nouns) {
  if (DEFER.has(id)) continue
  const rows = (OVERRIDES[id] || tables[id]).slice().sort((a, b) => order.indexOf(a.tag) - order.indexOf(b.tag))
  const en = DICT[id].en
  const body = rows.map((r) => `    { al: '${r.al}', tag: '${r.tag}', gloss: ${JSON.stringify(r.gloss)} },`).join('\n')
  final[id] = `  ${id}: [ /* ${en}${OVERRIDES[id] ? ' — explicit override' : ''} */\n${body}\n  ],`
}
...
console.log('  (explicit overrides:', Object.keys(OVERRIDES).filter((id) => final[id]).length, ', deferred:', [...DEFER].join(', '), ')')
console.log('flagged during inference:', flagged.length)
