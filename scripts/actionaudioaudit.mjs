...
await check('the app keeps the source scene inert until karaoke playback settles', () => {
  ...
  assert.match(app, /const commitAcceptedAction = useCallback\([\s\S]*?baseDispatch\(action\)[\s\S]*?captureCommittedTransition\(action, before, after\)/)
  assert.match(app, /const finishActionTransition = useCallback\([\s\S]*?commitAcceptedAction\(transition\.action, current, reducer\(current, transition\.action\)\)/)
