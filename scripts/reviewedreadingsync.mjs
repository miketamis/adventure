...
for (const [owner, tranche] of tranches.entries()) {
  for (const [oldAddress, compactReview] of Object.entries(tranche.values)) {
    const review = Array.isArray(compactReview)
      ? { al: compactReview[0], en: compactReview[1] }
      : compactReview
...
if (write) {
  for (const [owner, tranche] of tranches.entries()) {
    const pairOf = (review) => `[${JSON.stringify(review.al)}, ${JSON.stringify(review.en)}]`
    const counts = new Map()
    for (const [, review] of next[owner]) {
      const pair = pairOf(review)
      counts.set(pair, (counts.get(pair) || 0) + 1)
    }
    const factories = new Map()
    for (const [pair, count] of counts) {
      if (count < 3) continue
      const name = `repeatedReading${factories.size}`
      const call = `${name}()`
      const declaration = `const ${name} = () => ${pair}\n`
    ...
    const body = next[owner]
      .map(([address, review]) => {
        const pair = pairOf(review)
        return `  ${JSON.stringify(address)}: ${factories.get(pair)?.call || pair},`
      })
      .join('\n')
