...
import { reviewedReadingPartitionSource } from './lib/reviewed-readings-format.mjs'
...
if (write) {
  for (const [owner, tranche] of tranches.entries()) {
    writeFileSync(fileURLToPath(tranche.path), reviewedReadingPartitionSource(tranche.name, next[owner]))
  }
}
