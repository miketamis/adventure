...
check('runtime and Debug Learning read timing and adaptation from the shared registries', () => {
  ...
  const preparation = fs.readFileSync('src/trainPreparation.js', 'utf8')
  ...
  assert.match(preparation, /const \{ state, nowMs: requestedAtMs = Date\.now\(\) \} = options/)
  assert.match(preparation, /nowMs: requestedAtMs,/)
  assert.match(preparation, /const \{ enumeration, plannedAtMs: nowMs, validUntilMs \} = bank/,
    'reused candidate urgency no longer retains the time used for its eligibility decision')
