...
const LANDMARK_IDS = new Set(WORLD_LANDMARKS.map((landmark) => landmark.id))
...
// Mirrored horizontally (x → 1024 − x) so the town faces the river and bridge
// on the chart's left, toward the great forest beyond it.
...
// explicitly authored "what can happen here" list (see VillageMap / placeMeta.js).
...
// ---------------------------------------------------------------------------
// THE WORLD beyond the village — every OTHER node belongs to a terrain region,
// so the full story is visible on one pan/zoom chart. Regions ring the village:
// sky & mountain above, forest left, river/castle/sea right and below, then the
// underworld down through the well.
// ---------------------------------------------------------------------------
const VILLAGE_IDS = new Set(VILLAGE_PLACES.map((p) => p.id))
// ── THE MYTHIC COMPOSITE TALE CHART ────────────────────────────────────────
// This is not a projection or a rotated survey map. It preserves named Albanian
// places as distinct tale-spaces while arranging playable roads around the
// village; the sky and world below use a cosmological vertical axis.
function hashStr(s) {
  let h = 2166136261
...
const SEA_FAR = 4800
// A stylised Albanian Adriatic coast for the composite chart: a headland, a
// broad Drin-like bay and a Buna-like delta. These are visual motifs rather than
// a claim of surveyed latitude, cardinal orientation or scale.
const seaCoastX = (y) => {
...
function terrSea() {
  ...
  return (
    <g>
      ...
      {/* Old-chart dressing, labelled with the chart's SEMANTIC axes. This is
          deliberately not a compass: the tale chart has no cardinal projection. */}
      <g transform="translate(1400,-210)" opacity={0.75} style={{ pointerEvents: 'none' }}>
        ...
        <text y={-54} textAnchor="middle" fontSize={12} fontStyle="italic" fontFamily="Georgia, serif" fill="#f4f9fb" opacity={0.9}>MAL</text>
        <text y={70} textAnchor="middle" fontSize={10} fontStyle="italic" fontFamily="Georgia, serif" fill="#eaf3f7" opacity={0.8}>NËNTOKË</text>
        <text x={-62} y={5} textAnchor="middle" fontSize={11} fontStyle="italic" fontFamily="Georgia, serif" fill="#eaf3f7" opacity={0.8}>PYLL</text>
        <text x={60} y={5} textAnchor="middle" fontSize={11} fontStyle="italic" fontFamily="Georgia, serif" fill="#eaf3f7" opacity={0.8}>DET</text>
      </g>
...
function terrCavern(rg) {
  ...
  // ruined towers and silent stocked bazaar inside the gate, the Beauty of the
  // Earth's silver grove, the Kulshedra's ember pit, and a cold river of the
  // dead threading the floor. Living Gjakova is drawn on the surface instead.
  ...
  return (
    <g>
      ...
      {/* the empty street: in from the gate, through the ruins, down to the silent bazaar */}
      <path d="M 380 2070 C 350 2062 322 2058 300 2062 C 330 2088 374 2140 430 2210"
      ...
      {/* Durham's bazaar is stocked but wholly deserted: goods, no hearths or inhabitants. */}
      <g>
        <path d={blobPath(430, 2210, 76, 42, 313, 0.1, 12)} fill="#38312c" stroke="#51483e" strokeWidth={2} />
        {[[392, 2190], [462, 2188], [405, 2230], [470, 2228]].map(([fx, fy], i) => (
          <ellipse key={'fs' + i} cx={fx} cy={fy} rx={8} ry={4.4} fill="#6a6156" opacity={0.5} />
        ))}
        {stall(410, 2204)}{stall(454, 2218)}
      </g>
...
// The network: from the crossroads (top of the village) a road climbs to Mount
// Tomorr; the village high-street follows the river toward the castle country;
// the FOREST road crosses left over the tanners'
// stone bridge (the town's one finished crossing) and wanders to the great
// forest; and a coast road runs right from the river-mouth to the sea.
...
// The chart's single river-road: from the Tomorr side, past the village's left
// edge, bridge/mill/spring and the Zana's reach, toward the castle country.
...
export function VillageMap({ g, current, objective = null, goGraph, compact, follow, world, npcs, rumors, jumpTo, player = false, known = [] }) {
  ...
  const { dots, edges, oddEdges, clusters, singles, pos } = useMemo(() => {
    ...
    // EVERY node's position is explicit in NODE_POS. Nodes sharing a
    ...
    // SAME SPOT = explicitly authored in nodePositions.js: a node either owns its
  ...
  // The docked minimap follows the player and should mute unrelated routes.
  // The full review map is an atlas: keep the whole world legible until the
  // reviewer deliberately selects a scene, while still ringing the current
  // position below.
  const focus = sel || (follow ? current : null)
  const nbr = useMemo(() => {
  ...
  const isObjectivePlace = (id) => Boolean(objective) && placeOf(id) === placeOf(objective)
  const knownAtPlace = (id) => [...knownSet].find((knownId) => placeOf(knownId) === placeOf(id)) || null
  const canInspect = (id) => !player || Boolean(knownAtPlace(id))
  const inspectionTarget = (id) => isCurrentPlace(id) ? current : isObjectivePlace(id) ? objective : knownAtPlace(id) || id
  ...
  useEffect(() => {
    ...
    if (!p) return
    setSel(current)
    const target = objective && pos[objective]
    if (target && (target[0] !== p[0] || target[1] !== p[1])) {
      // During a role detour, show both the traveller's present position and
      // the waiting tale scene without asking the player to hunt or zoom out.
      const dx = Math.abs(target[0] - p[0])
      const dy = Math.abs(target[1] - p[1])
      const k = Math.max(0.16, Math.min(1.4, 850 / Math.max(1, dx), 540 / Math.max(1, dy)))
      const cx = (p[0] + target[0]) / 2
      const cy = (p[1] + target[1]) / 2
      setView({ x: 580 - cx * k, y: 380 - cy * k, k })
      return
    }
    const k = 1.4 // zoom in close on the current scene (the minimap follows the player)
    setView({ x: 580 - p[0] * k, y: 380 - p[1] * k, k })
  }, [follow, current, objective, pos])
  ...
  const keyboardMarkerRef = useRef(null)
  const mapMarkerProps = (markerKey, sceneId, x, y, activate, enabled = true) => {
    if (!enabled) return {}
    const markerId = `${markerKey}:${x}:${y}`
    return {
      role: 'button',
      ...
      'data-map-y': y,
      onFocus: (event) => {
        keyboardMarkerRef.current = markerId
        for (const marker of svgRef.current?.querySelectorAll('[data-map-marker="true"]') || []) {
          marker.setAttribute('tabindex', marker === event.currentTarget ? '0' : '-1')
        }
      },
      onKeyDown: (event) => {
        ...
        const nodes = [...(svgRef.current?.querySelectorAll('[data-map-marker="true"]') || [])]
        ...
        const next = nextMapMarker(markers, markerId, event.key)
        if (!next) return
        event.preventDefault()
        next.node.focus()
      },
    ...
    <div className="dbg-map">
      {!compact && (
        <p className="dbg-note">
          ...
          not a scaled or cardinal map of Albania. Named places remain distinct, while playable tale-roads arrange
          <b> Mount Tomorr and the sky</b> above, the <b>great forest</b> left, the <b>river, castle and lake country</b>
      ...
      <div className="dbg-worldwrap">
        <div className="dbg-worldtools">
          ...
          {!player && <button className={'btn' + (showOdd ? ' active' : '')}
          ...
          {worldStatus && <span className="dbg-world-status" title="live world state">◷ {worldStatus}</span>}
        </div>
        <svg ref={svgRef} viewBox="0 0 1160 760" className="dbg-world" preserveAspectRatio="xMidYMid meet"
             aria-label="Interactive world map. Use arrow keys to move between nearby known places and Enter to select."
          ...
          <g ref={panGRef} transform={`translate(${view.x} ${view.y}) scale(${view.k})`}>
            ...
            <g>
              ...
              {VILLAGE_PLACES.map((pl) => {
                ...
                const isSel = placeOf(pl.id) === placeOf(sel), isCur = isCurrentPlace(pl.id)
                const objectiveAtPlace = isObjectivePlace(pl.id)
                const dim = focus && nbr && !nbr.has(pl.id)
                const target = inspectionTarget(pl.id)
                return (
                  <g key={pl.id} className="dbg-vpin" opacity={dim ? 0.3 : 1}
                     {...mapMarkerProps(`village-${pl.id}`, pl.id, pl.x, pl.y, () => setSel(target), canInspect(pl.id))}
            ...
            <g>
              {edges.map(([x1, y1, x2, y2, u, v, wander], i) => {
                if (player && (!knownSet.has(u) || !knownSet.has(v))) return null
            ...
            {singles.map((d) => {
              ...
              if (player && !canInspect(d.id)) return null
              const isSel = placeOf(d.id) === placeOf(sel), isCur = isCurrentPlace(d.id)
              const isObjective = isObjectivePlace(d.id)
              const dim = focus && nbr && !nbr.has(d.id)
              const label = isSel || (nbr && nbr.has(d.id)) || hubSet.has(d.id)
              const target = inspectionTarget(d.id)
              return (
                <g key={d.id} className="dbg-wdot" opacity={dim ? 0.22 : 1}
                  ...
                  {isObjective && <><circle cx={d.x} cy={d.y} r={15.5} fill="none" stroke="#c9a7f2" strokeWidth={2.2} opacity={0.95} /><circle cx={d.x} cy={d.y} r={19} fill="none" stroke="#765b98" strokeWidth={1.3} strokeDasharray="3 3" opacity={0.9} /></>}
            ...
            {clusters.map((c) => {
              if (player && !c.members.some((m) => knownSet.has(m.id))) return null
              const knownMember = player && c.members.find((m) => knownSet.has(m.id))
              const host = c.hostVp || c.hostLm
              const clusterInteractive = !player || !host || !canInspect(host)
              const dimAll = focus && nbr && !c.members.some((m) => nbr.has(m.id))
              const holds = c.members.some((m) => placeOf(m.id) === placeOf(sel) || isCurrentPlace(m.id))
              const holdsObjective = c.members.some((m) => isObjectivePlace(m.id))
              const hot = holds || holdsObjective || openPlace === c.key
              const ctrl = host ? [c.x + 13, c.y - 15] : [c.x, c.y]
              const target = inspectionTarget(knownMember?.id || c.members[0].id)
              const activate = () => player
                ? setSel(target)
                : setOpenPlace(openPlace === c.key ? null : c.key)
              return (
                <g key={c.key} className="dbg-wdot" opacity={dimAll ? 0.4 : 1}
                  ...
                  {holds && !host && <circle cx={c.x} cy={c.y} r={13} fill="none" stroke={c.members.some((m) => m.id === sel) ? '#3ad0c0' : '#fff'} strokeWidth={2} opacity={0.8} />}
                  {holdsObjective && !host && <><circle cx={c.x} cy={c.y} r={16.5} fill="none" stroke="#c9a7f2" strokeWidth={2.2} opacity={0.95} /><circle cx={c.x} cy={c.y} r={20} fill="none" stroke="#765b98" strokeWidth={1.3} strokeDasharray="3 3" opacity={0.9} /></>}
                  {host
            ...
            <g style={{ pointerEvents: 'none' }} opacity={world?.phase === 'night' ? 0.08 : world?.phase === 'dusk' || world?.phase === 'dawn' ? 0.55 : 1}>
            ...
            {WORLD_LANDMARKS.map((lm, i) => {
              ...
              const isSel = placeOf(lm.id) === placeOf(sel), isCur = isCurrentPlace(lm.id)
              const objectiveAtPlace = isObjectivePlace(lm.id)
              const dim = focus && nbr && !nbr.has(lm.id)
              const target = inspectionTarget(lm.id)
              return (
                <g key={'lm' + i} className="dbg-vpin" opacity={dim ? 0.28 : 1}
                   {...mapMarkerProps(`landmark-${lm.id}`, lm.id, lm.x, lm.y, () => setSel(target), canInspect(lm.id))}
        ...
        {placeCard && !player && (
          <div className="dbg-worldinfo dbg-placecard">
            ...
            {goGraph && <button className="btn dbg-wi-open" onClick={() => goGraph(placeCard.anchor)}>open in Story Graph →</button>}
          </div>
        )}

        {!placeCard && info && !player && (
          <div className="dbg-worldinfo">
            ...
            {goGraph && <button className="btn dbg-wi-open" onClick={() => goGraph(sel)}>open in Story Graph →</button>}
          </div>
        )}

        {!placeCard && info && player && canInspect(sel) && (
          <div className="dbg-worldinfo atlas-location">
            <div className="dbg-wi-head">
              <b>{isCurrentPlace(sel) ? 'You are here' : isObjectivePlace(sel) ? 'Your character tale waits here' : 'Known story place'}</b>
              <button className="dbg-wi-x" title="clear selection" onClick={() => setSel(null)}>✕</button>
            </div>
            {info.firstLine && <p className="dbg-wi-line">“{info.firstLine}”</p>}
            <p className="dbg-wi-line">Return to Story to travel from here by the choices in the scene.</p>
          </div>
        )}
      </div>
    </div>
