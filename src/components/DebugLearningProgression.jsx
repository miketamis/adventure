        ...
        <div className="dbg-learning-lanes">
          ...
          <section className="dbg-learning-rules dbg-learning-foot" aria-labelledby="dbg-learning-scheduling-title">
            ...
            <ul>
              ...
              <li><b>Story-action goal:</b> {TRAIN_ACTION_GOAL_POLICY.priority}. When legal alternatives exist, each missing requested-action word waits for {TRAIN_ACTION_GOAL_POLICY.minimumNonGoalActivitiesBeforeTokenOpportunity} non-goal activities, then receives a token opportunity within {TRAIN_ACTION_GOAL_POLICY.maximumActivitiesPerTokenOpportunity} completed activities; after {TRAIN_ACTION_GOAL_POLICY.maximumNonGoalActivitiesBeforeForcedOpportunity} non-goal rounds, ordinary spacing and diversity rules yield to it. Once it is funded, missing-token words from other same-node actions whose vocabulary is saved become the next priority.</li>
              <li>The planner uses the last {TRAIN_QUESTION_MIX_POLICY.activityBalance.historyWindow} activity formats as diversity history. It may reuse a format only with disjoint Albanian language; only after the requested and alternate-action pools have no certified legal proposal does Train say “{TRAIN_ACTION_GOAL_POLICY.terminalMessage}”</li>
              ...
              <li>{TRAIN_SCHEDULER_SAFEGUARDS.noImmediateSharedWords && 'Ordinary consecutive questions never share an Albanian word.'} A reviewed card for a still-missing visible-action word is the sole last-resort repeat exception; it outranks the terminal screen after every ordinary future route is exhausted. {!TRAIN_SCHEDULER_SAFEGUARDS.repeatWhenNoDisjointTargetExists && `Only after every visible-action token target is funded may Train report ${TRAIN_SCHEDULER_SAFEGUARDS.exhaustedPoolOutcome}.`}</li>
