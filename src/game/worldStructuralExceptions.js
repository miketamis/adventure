...
export const STRUCTURAL_EXCEPTIONS = Object.freeze([
  ...
  reviewedStructuralException({
    id: 'gjizar-rescuers-homeward-return',
    rule: 'route-distance',
    targets: ['gjizarTradheti->gjizarKthim'],
    maximumTargets: 1,
    rationale: 'The chosen climb leaves the well with the king’s rescuers, whose exact arrival consequence then explicitly brings the prince home. This source-bound return includes the intervening journey and recovery; it is not a climb across the map.',
    source: 'content: gjizar-climb-king-rope and gjizarKthim.text[0..1]; Gjizar source topat 10.9–10, where the king sends men who retrieve the prince and bring him home',
    owner: 'story-continuity',
    reviewTrigger: 'when the rescue action, exact homeward-arrival prose, source account or physical endpoints change, or the return gains playable travel scenes',
  }),
  reviewedStructuralException({
    id: 'gjizar-recovery-home-uncharted-exit',
    rule: 'stranded-node',
    targets: ['gjizarKthim'],
    maximumTargets: 1,
    rationale: 'The king’s home has one distant charted neighbor, the betrayal well, reached by the source-bound rescue return and two-day recovery. Its ordinary onward action rides to the Beauty’s uncharted ship; adding a nearby charted road would invent a location the source does not supply.',
    source: 'Gjizar topat 10.9–10 and kenga 11.1–11.8; gjizar-climb-king-rope, gjizarKthim and the exact gjizar-ride-to-ship transition in UNCHARTED_SITES',
    owner: 'story-continuity',
    reviewTrigger: 'when this home gains another charted neighbor, its source rescue or recovery timing changes, or the ship acquires a source-supported chart anchor',
  }),
  reviewedStructuralException({
