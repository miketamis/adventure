...
export const STRUCTURAL_EXCEPTIONS = Object.freeze([
  ...
  reviewedStructuralException({
    ...
    targets: ['pemaDielli->rrugaDielli2'],
    maximumTargets: 1,
    rationale: 'The player speaks at the tree and the stag then carries the maiden over the entire road home before the target scene opens.',
    source: 'content: pemaDielli option and rrugaDielli2 arrival',
    owner: 'narrative',
    reviewTrigger: 'when the flight home gains intermediate journey scenes',
  }),
  reviewedStructuralException({
    ...
    maximumTargets: 4,
    rationale: 'These local-looking perception or conversation actions deliberately cross into an embodied tale setting instead of pretending the distant chart vector is a walk.',
  ...
  reviewedStructuralException({
    id: 'moon-terrace-deliberate-separation',
    rule: 'stranded-node',
    targets: ['henaPaqe'],
    maximumTargets: 1,
    rationale: 'The Moon’s terrace intentionally stands across the sky from the Sun, and the authored search choice names that long crossing.',
    source: 'content: qiellDiell to henaPaqe search; nodePositions: henaPaqe',
    owner: 'world-map',
    reviewTrigger: 'when the Moon search gains an intermediate sky-road node',
  }),
  reviewedStructuralException({
  ...
  reviewedStructuralException({
    ...
    maximumTargets: 3,
    rationale: 'These exact roads follow the river between the crossroads, distant sea and Lake Shkodra, with each choice naming the long onward journey.',
  ...
  reviewedStructuralException({
    ...
    rule: 'route-distance',
    targets: ['shqipe1->shtepia'],
    maximumTargets: 1,
    rationale: 'The option explicitly sends the traveller far home from the eagle’s tree, so the long route is the promised journey.',
    source: 'content: shqipe1 far-home choice and shtepia arrival; nodePositions: exact vector',
    owner: 'narrative',
    reviewTrigger: 'when the eagle-tree homecoming gains an intermediate road scene',
  }),
  reviewedStructuralException({
  ...
  reviewedStructuralException({
    ...
    rule: 'route-distance',
    targets: ['qiellDiell->henaPaqe', 'qiell2->qiellPrende', 'qiellErera2->qiell2'],
    maximumTargets: 3,
    rationale: 'These exact sky passages are the deliberate Moon search, the fall from the storm peak and the winds carrying the player upward with closed eyes.',
    source: 'content: qiellDiell, qiell2 and qiellErera2 transition prose; nodePositions: exact vectors',
    owner: 'world-map',
  ...
  reviewedStructuralException({
    ...
    targets: ['kafeneja->fshatiSheshi', 'kafeneja2->fshatiSheshi'],
    maximumTargets: 2,
    rationale: 'These return labels cross from the coffee-house interior onto the square even though both scenes intentionally share one compact map place.',
    source: 'content: kafeneja exits; place model: shared square anchor',
    owner: 'narrative',
