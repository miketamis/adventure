...
// The live node view needs canonical geometry and story sources, not the
// complete historical/source-tale catalogue. The atlas supplies that inventory.
export function buildWorldScene3dCore(inventory = { elements: [], descriptions: [], bindings: [], records: [], coverage: {} }) {
  const elements = []
  const descriptions = []
  const routes = []
  const relations = []
 ...
 */
export function validateWorldScene3dCore(model, inventoryDefinition, inventoryIssues) {
  const issues = []
  ...
  issues.push(...inventoryIssues, ...validateWorldScene3dPlaceProfiles(model))
