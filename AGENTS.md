...
## Holistic A2 learning evidence

- Treat A2 as a non-compensatory profile across listening, reading, spoken interaction, spoken production, written interaction, written production and mediation. Tokens, vocabulary totals, story completion or strength in one mode must never stand in for another mode.
- Keep exact learning, guided transfer, durable multi-context practice and fresh held-out readiness as distinct evidence. Preserve first attempts, space retrieval, and use delayed reconstruction or changed-context retelling where the capability requires transfer rather than immediate recall.
- Persist answer-support exposure as compact non-content evidence. Remounting, reloading or immediately retrying must not make a revealed item fresh again; a new fresh attempt requires an intervening activity with disjoint language targets.
- Count listening exposure only after the continuous clip finishes successfully. A click, muted no-op, interrupted clip, rejected playback or missing asset must not unlock an answer surface or award listening evidence.
- A communicative learning task must either act through a world-bound information, action or relationship gap with a canonical consequence, or be clearly labelled as low-stakes lore rehearsal whose evidence cannot claim world transfer. Do not add a detached duplicate quiz merely to increase activity count, and never label a static scenario card as the player's current story state.
- Treat unrestricted speech and writing as low-stakes diagnosis with task-specific feedback and revision unless independently validated. Keep the original and revised response in page memory long enough to support that revision, persist only non-content completion evidence, and do not turn deterministic surface checks into a holistic CEFR pass.
- Objective authored task floors such as required turns, minimum words and minimum sentences gate task fulfilment even when response quality remains self-reviewed. A required interlocutor turn must be an actual revealed follow-up that the learner answers, not a paragraph count the learner can manufacture alone.
- Author task-specific lexical anchors for deterministic open-response notices; never infer them from English prompts. Every offered revision button must lead to refreshed feedback or an explicit practice completion, and phase changes must announce themselves and move keyboard focus to the new work.
- ASR, accent scores and LLM judgments must not award CEFR readiness, durable mastery, progression or heart consequences without Albanian-specific validation and audited fairness, reliability and recovery. Fully open AI dialogue is never a required progression path by default.
- Publicly describe the outcome as an internally audited A1-ready/A2-ready profile, not fluency, accreditation or a guaranteed external-assessment pass; stronger claims require specialist review, learner pilots, human-rated samples and external standard-setting.
